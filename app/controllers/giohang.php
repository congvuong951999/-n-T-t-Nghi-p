<?php 
	...
	class giohang extends DController{
			...
			if(isset($_POST['delete_cart'])){
				...
				foreach($_POST['qty'] as $key => $qty){
					foreach($_SESSION["shopping_cart"] as $session => $values){
						...
						//kiem tra so luong
						// $addProduct = mysqli_query($db, "SELECT `product_quantity` FROM `tbl_product` WHERE `id_product` =".$product_id);
		...
		public function updategiohangg(){
			Session::init();
			if(isset($_GET['cong'])){
			$product_id=$_GET['cong'];
			foreach($_SESSION['shopping_cart'] as $value){
				if($value['product_id']!=$product_id){
					$product[]= array('product_title'=>$value['product_title'],'product_id'=>$value['product_id'],'product_quantity'=>$value['product_quantity'],'product_price'=>$value['product_price'],'product_image'=>$value['product_image']);
					$_SESSION['shopping_cart'] = $product;
				}else{
					$tangsoluong = $value['product_quantity'] + 1;
					if($value['product_quantity']<=9){
						
						$product[]= array('product_title'=>$value['product_title'],'product_id'=>$value['product_id'],'product_quantity'=>$tangsoluong,'product_price'=>$value['product_price'],'product_image'=>$value['product_image']);
					}else{
						$product[]= array('product_title'=>$value['product_title'],'product_id'=>$value['product_id'],'product_quantity'=>$value['product_quantity'],'product_price'=>$value['product_price'],'product_image'=>$value['product_image']);
					}
					$_SESSION['shopping_cart'] = $product;
