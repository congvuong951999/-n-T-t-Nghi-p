...
<?php 
	...
	class customermodel extends DModel{
		...
		public function customer_product($table_customer){
			$sql = "SELECT * FROM $table ORDER BY customer_id  DESC";
			return $this->db->select($sql);
		}
		public function list_user_home($table_customer){
			$sql = "SELECT * FROM $table_customer ORDER BY $table_customer.customer_id  DESC";
			return $this->db->select($sql);
		}
		//category product
